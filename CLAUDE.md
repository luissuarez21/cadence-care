# Cadence — Full Project Guide (CLAUDE.md)
...
OB uploads care plan
        ↓
Claude (vision) ingests → structured protocol JSON
...
- **Agent orchestrator:** Anthropic Claude API with tool use
  - Model: `claude-sonnet-4-6` (latest capable model — do NOT hardcode older versions)
...
- **Plan ingestion pipeline:**
  - Upload endpoint → Claude vision reads PDF/image → structured protocol JSON
...
- **Risk engine:** `assess_risk` tool loads the condition pack's `red_flags[]`, evaluates current
  session data against thresholds, calls Claude with a structured rubric → returns
...
- **Visit summary generation:** `generate_visit_summary` queries all symptom logs and risk
  timeline for the period since last appointment → Claude summarizes → two outputs:
...
- In production: all LLM inference moves to **Amazon Bedrock** (Claude models available on
...
- **LLM-as-judge safety eval** — after every escalation, a second Claude call independently
...
- **Adit:** Agent loop (Claude tool use), plan ingestion (vision → JSON), Redis memory,
...
- FastAPI backend
- Claude orchestrator with tool use (all 6 tools implemented)
- Plan ingestion pipeline (Claude vision → Pydantic-validated JSON)
...
|---|---|---|
| **Anthropic Claude** | Core LLM orchestrator + vision for plan ingestion + tool use | Anthropic health track ✅ |
...
7. **Use the latest Claude model.** `claude-sonnet-4-6` for all LLM calls unless a specific reason
...
│   ├── agent/
│   │   ├── orchestrator.py         ← Claude tool-use loop (Adit)
...
│   ├── ingestion/
│   │   ├── pipeline.py             ← upload → Claude vision → ProtocolJSON (Adit)
...
Greenfield as of June 2026. CLAUDE.md reflects the full product vision after advisor review
