...
def respond(
    ...
    # One root span per request, so the whole turn nests into a single Arize
    # waterfall: cadence.request → safety.classify → orchestrator.turn → tool.* →
    # (safety/care-plan escalation). Without this each piece is a scattered root span.
    with agent_span("cadence.request", tracer=_TRACER, patient_id=patient_id):
        # Semantic safety layer (condition-agnostic). Runs on every message BEFORE
        # the care-plan agent loop. Catches danger the condition pack can't encode
        # (self-harm, abuse, acute emergency) and routes it to the clinician
        # regardless of assess_risk.
        ...
        if safety.is_escalating(verdict):
            try:
                ...
                # A safety escalation must never crash the turn; verdict is still traced.
                pass
        ...
        for _ in stream_agent(patient_id, system_prompt, messages, result, tracer=_TRACER):
            pass
        # Lower-severity safety concern (e.g. acute distress): the clinician was
        # notified above; mark the turn flagged so the UI reflects it.
        if safety.is_escalating(verdict):
            result.flagged = True
        return result
